...
function setView(view){
  ...
  const meta={
    ...
    staff:["員工前台","員工用編號查看班表與填寫可排時間"]
  }[view];
  byId("pageTitle").textContent=meta[0];byId("pageSubtitle").textContent=meta[1];
  byId("sidebar").classList.remove("open");
  renderAll();
}
function renderAll(){renderDashboard();renderEmployees();renderWorktypes();renderCalendar();renderTimeline();renderStaff()}
...
function renderStaff(){
  if(!state.staffEmployeeId)return;
  const e=employee(state.staffEmployeeId);if(!e)return;
  byId("staffWelcome").textContent=`${e.name}，你好`;
  const shifts=state.data.shifts.filter(s=>s.employeeId===e.id).sort((a,b)=>a.date.localeCompare(b.date));
  byId("staffShiftList").innerHTML=shifts.length?shifts.map(s=>`<div class="list-item"><div class="list-icon" style="background:${worktype(s.workTypeId).color}22;color:${worktype(s.workTypeId).color}">●</div><div class="list-main"><strong>${formatDate(s.date)}｜${worktype(s.workTypeId).name}</strong><span>${s.start}～${s.end}・計薪 ${fmtHours(durationHours(s))}</span></div></div>`).join(""):`<div class="empty-state">目前沒有班表</div>`;
  const av=state.data.availability.filter(a=>a.employeeId===e.id).sort((a,b)=>a.date.localeCompare(b.date));
...
function openShiftModal(id=null){
  const s=id?state.data.shifts.find(x=>x.id===id):{id:uid("s"),date:state.selectedDate,employeeId:state.data.employees.find(e=>e.active)?.id||"",workTypeId:state.data.workTypes.find(w=>w.active)?.id||"",start:"09:00",end:"17:00",breakMinutes:0,note:"",prepRole:false,status:"draft"};
  const w=worktype(s.workTypeId);if(!id&&w)s.breakMinutes=w.defaultBreak;
  openModal(id?"編輯班次":"新增班次","時間、休息與計薪工時皆可單筆調整",`
  <div class="form-grid">
    ...
    <label class="field span-2"><span>員工</span><select class="select" name="employeeId">${state.data.employees.filter(e=>e.active).map(e=>`<option value="${e.id}" ${e.id===s.employeeId?"selected":""}>${e.name}（${e.employeeNo}）</option>`).join("")}</select></label>
    <label class="field"><span>開始</span><select class="select" name="start">${timeOptions(s.start)}</select></label>
    <label class="field"><span>結束</span><select class="select" name="end">${timeOptions(s.end)}</select></label>
  ...
  function updateWarnings(){const fd=new FormData(form),eid=fd.get("employeeId"),date=fd.get("date"),start=fd.get("start"),end=fd.get("end"),br=Number(fd.get("breakMinutes")||0),e=employee(eid),warnings=[];
    if(mins(end)<=mins(start))warnings.push("結束時間必須晚於開始時間");
    const a=state.data.availability.find(x=>x.employeeId===eid&&x.date===date);
    if(a?.unavailable)warnings.push("此員工當天填寫為不可排班");
    else if(a&&(start<a.start||end>a.end))warnings.push(`超出員工可排時間 ${a.start}～${a.end}`);
    const temp={...s,start,end,breakMinutes:br};const week=weeklyHours(eid,date,id)+durationHours(temp);if(e?.weeklyLimit&&week>e.weeklyLimit)warnings.push(`排入後本週 ${fmtHours(week)}，超過上限 ${e.weeklyLimit} 小時`);
    const overlap=state.data.shifts.some(x=>x.id!==id&&x.employeeId===eid&&x.date===date&&mins(start)<mins(x.end)&&mins(end)>mins(x.start));if(overlap)warnings.push("與此員工當天其他班次重疊");
    byId("shiftWarnings").innerHTML=warnings.map(w=>`<div class="list-item"><div class="list-icon">⚠</div><div class="list-main"><strong>${w}</strong><span>第一版允許主管仍然儲存</span></div></div>`).join("")
  }
  form.onchange=updateWarnings;updateWarnings();
  form.onsubmit=ev=>{ev.preventDefault();const fd=new FormData(ev.target);Object.assign(s,{date:fd.get("date"),workTypeId:fd.get("workTypeId"),employeeId:fd.get("employeeId"),start:fd.get("start"),end:fd.get("end"),breakMinutes:Number(fd.get("breakMinutes")||0),prepRole:fd.get("prepRole")==="on",note:fd.get("note").trim()});if(mins(s.end)<=mins(s.start)){alert("結束時間必須晚於開始時間");return}if(!id)state.data.shifts.push(s);state.selectedDate=s.date;save();closeModal()}
}
...
function init(){
  ...
  byId("staffLoginBtn").onclick=()=>{const no=byId("staffNoInput").value.trim().toUpperCase(),e=state.data.employees.find(x=>x.active&&x.employeeNo.toUpperCase()===no);if(!e){byId("staffLoginError").textContent="找不到此員工編號，請確認後再試。";return}state.staffEmployeeId=e.id;byId("staffLoginCard").classList.add("hidden");byId("staffPortal").classList.remove("hidden");renderStaff()};
  byId("staffLogoutBtn").onclick=()=>{state.staffEmployeeId=null;byId("staffPortal").classList.add("hidden");byId("staffLoginCard").classList.remove("hidden");byId("staffNoInput").value=""};
  ...
  byId("availabilityStart").innerHTML=timeOptions("16:00");byId("availabilityEnd").innerHTML=timeOptions("22:00");byId("availabilityDate").value=state.selectedDate;
  ...
  byId("saveAvailabilityBtn").onclick=()=>{if(!state.staffEmployeeId)return;const date=byId("availabilityDate").value,unavailable=byId("availabilityUnavailable").checked,start=byId("availabilityStart").value,end=byId("availabilityEnd").value;if(!date){alert("請選日期");return}if(!unavailable&&mins(end)<=mins(start)){alert("結束時間必須晚於開始時間");return}let a=state.data.availability.find(x=>x.employeeId===state.staffEmployeeId&&x.date===date);if(!a){a={id:uid("a"),employeeId:state.staffEmployeeId,date};state.data.availability.push(a)}Object.assign(a,{unavailable,start,end});save();byId("availabilitySaved").textContent="已儲存";setTimeout(()=>byId("availabilitySaved").textContent="",1600)};
